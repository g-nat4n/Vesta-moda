      ...
      {BRAND.journal.articles.map((article, index) => {
        ...
        return (
          <Reveal key={article.title} delay={index * 0.08}>
            <article className="overflow-hidden border border-gold/70 bg-ivory transition hover:-translate-y-0.5 hover:shadow-[0_8px_18px_rgba(23,22,17,0.08)]">
            <div className="relative h-56">
              <Image src={article.image} alt={article.alt} fill className="object-cover" />
            </div>
            <div className="p-6">
              <p className="text-xs font-bold uppercase tracking-[0.16em] text-burgundy">{article.category}</p>
